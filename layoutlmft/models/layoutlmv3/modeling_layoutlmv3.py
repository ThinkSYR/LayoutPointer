...
from .modeling_re import REDecoder, ReOutput
from .modeling_mlre_decoder import MLREDecoder
...
class LayoutLMv3ForRelationExtraction(LayoutLMv3PreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = self.dropout(sequence_output)
        loss, batch_logits_all, pred_relations = self.extractor(sequence_output, entities, relations)
...
class LayoutLMv3ForMultiRelationExtraction(LayoutLMv3PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    # _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def __init__(self, config):
        ...
        self.layoutlmv3 = LayoutLMv3Model(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.extractor = MLREDecoder(config)
