...
from .configuration_gplayoutlmv3 import GPLayoutLMv3Config
from timm.models.layers import to_2tuple

from .modeling_gpdecoder import ReOutput
from .modeling_gpdecoder import GlobalPointer # EfficientGlobalPointer as
from .modeling_gpdecoder import GPREDecoder, GPMultiLineRecognitionREDecoder
from .modeling_lpdecoder import SERPointer, SCAREDecoder
from .modeling_mlre_lpdecoder import SCAMLREDecoder
from .modeling_gpdecoder import SER_MC_CE_loss
logger = logging.get_logger(__name__)


@dataclass
class BaseModelOutputWithPastAndCrossAttentionsAndBias(ModelOutput):
...
class BaseModelOutputWithPoolingAndCrossAttentionsAndBias(ModelOutput):
    last_hidden_state: torch.FloatTensor = None
    ...
    cross_attentions: Optional[Tuple[torch.FloatTensor]] = None
    bias: Optional[Tuple[torch.FloatTensor]] = None
    spatial_position_embeddings: torch.FloatTensor = None
...
class GPLayoutLMv3Embeddings(nn.Module):
    """
...
class GPLayoutLMv3PreTrainedModel(PreTrainedModel):
    """
    An abstract class to handle weights initialization and a simple interface for downloading and loading pretrained
    models.
    """

    config_class = GPLayoutLMv3Config
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            module.weight.data.normal_(mean=0.0, std=self.config.initializer_range)# TODO: BERT初始化固定0.02
...
class GPLayoutLMv3SelfAttention(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class GPLayoutLMv3Attention(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.self = GPLayoutLMv3SelfAttention(config)
...
class GPLayoutLMv3Layer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = GPLayoutLMv3Attention(config)
...
class GPLayoutLMv3Encoder(nn.Module):
    def __init__(self, config, detection=False, out_features=None):
        super().__init__()
        self.config = config
        self.detection = detection
        self.layer = nn.ModuleList([GPLayoutLMv3Layer(config) for _ in range(config.num_hidden_layers)])
    ...
    def _cal_2d_pos_emb(self, hidden_states, bbox):
        ...
        return rel_2d_pos, rel_pos_x + rel_pos_y #torch.cat([rel_pos_x, rel_pos_y], dim=1)
    ...
    def forward(
        ...
        rel_2d_pos, rel_2dpos_cat = self._cal_2d_pos_emb(hidden_states, bbox) if self.has_spatial_attention_bias else None
        ...
        return BaseModelOutputWithPastAndCrossAttentionsAndBias(
            last_hidden_state=hidden_states,
            past_key_values=next_decoder_cache,
            hidden_states=all_hidden_states,
            attentions=all_self_attentions,
            cross_attentions=all_cross_attentions,
            bias=rel_pos + rel_2dpos_cat,# torch.cat([rel_pos, rel_2dpos_cat], dim=1),
        )


class GPLayoutLMv3Model(GPLayoutLMv3PreTrainedModel):
    """
    """
    ...
    def __init__(self, config, detection=False, out_features=None, image_only=False):
        ...
        if not self.image_only:
            self.embeddings = GPLayoutLMv3Embeddings(config)
        self.encoder = GPLayoutLMv3Encoder(config, detection=detection, out_features=out_features)
    ...
    def forward(
        ...
        if not self.image_only:
            ...
            embedding_output, spatial_position_embeddings = self.embeddings(
                input_ids=input_ids,
                bbox=bbox,
        ...
        return BaseModelOutputWithPoolingAndCrossAttentionsAndBias(
            ...
            bias=encoder_outputs.bias,
            spatial_position_embeddings=spatial_position_embeddings,
        )


class GPLayoutLMv3ForTokenClassification(GPLayoutLMv3PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"position_ids"]

    def __init__(self, config):
        super().__init__(config)
        self.num_heads = config.num_labels

        self.layoutlmv3 = GPLayoutLMv3Model(config)
        
        if config.using_gp:
            self.classifier = GlobalPointer(config, heads=3)
        else:
            self.classifier = SERPointer(config)

        self.layoutlmv3.init_weights()
    ...
    def forward(
        ...
        logits = self.classifier(sequence_output, attention_mask, bbox)
        loss   = SER_MC_CE_loss(labels.clone(), logits)
        ###### classify ######
...
class GPLayoutLMv3ForRelationExtraction(GPLayoutLMv3PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"extractor_gpre"]
    ...
    def __init__(self, config):
        ...
        if config.using_gp:
            self.extractor_gpre = GPREDecoder(config)
        else:
            self.extractor_gpre = SCAREDecoder(config)

        self.layoutlmv3.init_weights()
        self.extractor_gpre.reset_parameters()
    ...
    def forward(
        ...
        re_outputs = self.extractor_gpre(sequence_output, entities, relations, attention_mask, bbox=bbox)

        attentions = outputs.attentions
        if len(re_outputs) == 3:
            loss, batch_logits_all, pred_relations = re_outputs
        else:
            loss, batch_logits_all, pred_relations, re_attentions = re_outputs
            if attentions is None:
                attentions = re_attentions
            else:
                attentions = attentions + re_attentions
            
        return ReOutput(
            loss=loss,
            logits=batch_logits_all,
            entities=entities,
            relations=relations,
...
class GPLayoutLMv3ForMultiRelationExtraction(GPLayoutLMv3PreTrainedModel):
    _keys_to_ignore_on_load_unexpected = [r"pooler"]
    _keys_to_ignore_on_load_missing = [r"extractor_gpre"]

    def __init__(self, config, mode="table"):
        super().__init__(config)

        self.layoutlmv3 = GPLayoutLMv3Model(config)
        if config.using_gp:
            self.extractor_gpre = GPMultiLineRecognitionREDecoder(config)
        else:
            self.extractor_gpre = SCAMLREDecoder(config)
    ...
    def forward(
        ...
        seq_length = input_ids.size(1)
        sequence_output, image_output = outputs[0][:, :seq_length], outputs[0][:, seq_length:]
        loss, batch_logits_all, pred_relations = self.extractor_gpre(sequence_output, entities, relations, attention_mask, bbox=bbox)
